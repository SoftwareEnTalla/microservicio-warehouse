...
@Injectable()
export class StorageLocationQueryService implements OnModuleInit{
  // Private properties
  readonly #logger = new Logger(StorageLocationQueryService.name);
  private readonly loggerClient = LoggerClient.getInstance();

  constructor(private readonly repository: StorageLocationQueryRepository,
  ...
  private validate(): void {
    try {
      ...
      if (!(entityInstance instanceof BaseEntity)) {
        let sms = `El tipo ${StorageLocation.name} no extiende de BaseEntity. Asegúrate de que todas las entidades hereden correctamente.`;
        logger.info(sms);
        throw new Error(sms);
      }
    } catch (error) {
  ...
  async findAll(
    ...
    try {
      const storagelocations = await this.repository.findAll(options);
      // Devolver respuesta
      logger.info("sms");
      return {
        ok: true,
        message: "Listado de storagelocations obtenido con éxito",
  ...
  async findById(id: string): Promise<StorageLocationResponse<StorageLocation>> {
    try {
      const storagelocation = await this.repository.findOne({
        where: { id },
        relations: [],
      });
  ...
  async findByField(
    ...
    try {
      ...
      if (!entities)
        throw new NotFoundException(
          "StorageLocations no encontrados para la propiedad y valor especificado"
        );
      // Devolver storagelocation
      return {
        ok: true,
        message: "StorageLocations obtenidos con éxito.",
        data: entities,
        pagination: Helper.getPaginator(
          paginationArgs ? paginationArgs.page : 1,
          paginationArgs ? paginationArgs.size : 25,
  ...
  async findOne(where?: Record<string, any>): Promise<StorageLocationResponse<StorageLocation>> {
    try {
