...
@ApiTags("Warehouse Query")
@UseGuards(WarehouseAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("warehouses/query")
export class WarehouseQueryController {
  #logger = new Logger(WarehouseQueryController.name);

  constructor(private readonly service: WarehouseQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all warehouse with optional pagination" })
  @ApiResponse({ status: 200, type: WarehousesResponse })
  @ApiQuery({ name: "options", required: false, type: WarehouseDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Warehouse>    
  ): Promise<WarehousesResponse<Warehouse>> {
    try {
     
      const warehouses = await this.service.findAll(options);
      logger.info("Retrieving all warehouse");
      return warehouses;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find warehouse by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter warehouse', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: WarehousesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Warehouse no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find warehouses with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Warehouse>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all warehouses" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count warehouses with conditions" })
  @ApiResponse({ status: 200, type: WarehousesResponse<Warehouse> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one warehouse with conditions" })
  ...
  @ApiOperation({ summary: "Find one warehouse or return error" })
  @ApiResponse({ status: 200, type: WarehouseResponse<Warehouse> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the warehouse to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<WarehouseResponse<Warehouse>> {
    try {
      const warehouse = await this.service.findOne({ where: { id } });
      if (!warehouse) {
        throw new NotFoundException(
